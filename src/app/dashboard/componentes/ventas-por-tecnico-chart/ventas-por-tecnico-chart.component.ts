...
@Component({
  selector: 'app-ventas-por-tecnico-chart',
  standalone: true,
  imports: [],
  templateUrl: './ventas-por-tecnico-chart.component.html',
  styleUrl: './ventas-por-tecnico-chart.component.css'
})
export class VentasPorTecnicoChartComponent {
 @ViewChild('chartCanvas') chartCanvas!: ElementRef<HTMLCanvasElement>;
  ...
  private loadChartData(): void {
    const data: TecnicoVentasAgrupadas[] = this.excelDataService.getExcelDataFacturadoTecnico();
  ...
  private processDataForChart(rawData: TecnicoVentasAgrupadas[]): void {
    const ventasPorTecnico: { [key: string]: number } = {};
    ...
    rawData.forEach(item => {
      ...
      if (tecnico && typeof monto === 'number' && !isNaN(monto)) {
        const tecnicosArray = tecnico.split(',').map(t => t.trim());
        tecnicosArray.forEach(t => {
          ventasPorTecnico[t] = (ventasPorTecnico[t] || 0) + (monto / tecnicosArray.length);
        });
      } else {
        console.warn(`VentasPorTecnicoChartComponent: Ítem omitido debido a datos inválidos: Técnico='${tecnico}', Monto='${monto}'`, item); // <--- LOG
      }
    });

    const labels = Object.keys(ventasPorTecnico);
    const data = Object.values(ventasPorTecnico);

    if (labels.length === 0 || data.length === 0) {
        console.warn('VentasPorTecnicoChartComponent: No hay datos procesados para el gráfico. Labels o Data están vacíos.');
        if (this.myChart) {
            this.myChart.destroy(); 
